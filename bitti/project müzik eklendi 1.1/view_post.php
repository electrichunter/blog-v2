...
if(isset($_POST['add_comment'])){
   ...
   $metinDosyasi = 'C:\\xampp\\htdocs\\project\\filtreleme-karaliste\\karaliste.txt'; // Küfür kelimelerinin bulunduğu dosyanın yolu

   $containsProfanity = false;
   
   // Küfür kelimelerini dosyadan oku
   $kufurKelimeListesi = file($metinDosyasi, FILE_IGNORE_NEW_LINES);
   
   foreach ($kufurKelimeListesi as $kufurKelime) {
       if (stripos($comment, $kufurKelime) !== false) {
   ...
   if ($containsProfanity) {
       $message[] = 'Argo içeren yorumlar engelleniyor!';
   } else {
...
if (isset($_POST['edit_comment'])) {
    ...
    // Düzenlenen yorumda küfür kontrolü yap
    $containsProfanity = false;

    // Küfür kelimelerini dosyadan oku
    $metinDosyasi = 'C:\xampp\htdocs\project\filtreleme-karaliste\karaliste.txt';

    $kufurKelimeListesi = file($metinDosyasi, FILE_IGNORE_NEW_LINES);

    foreach ($kufurKelimeListesi as $kufurKelime) {
        if (stripos($comment_edit_box, $kufurKelime) !== false) {
            $containsProfanity = true;
            break;
        }
    }
...
?>
   ...
   <form action="" method="POST">
      ...
      <button type="submit" class="inline-btn" name="edit_comment">yorumu düzenle</button>
      <div class="inline-option-btn" onclick="window.location.href = 'view_post.php?post_id=<?= $get_id; ?>';">editten çık</div>
   </form>
   </section>
<?php
   }
...
<section class="posts-container" style="padding-bottom: 0;">
   ...
   <div class="box-container">
      ...
      <?php
         ...
         echo '<p class="empty">gönderi yok!</p>';
      }
      ?>
   </div>
...
<section class="comments-container">
   ...
   <p class="comment-title">yorum ekle</p>
   <?php
      if($user_id != ''){  
   ...
   <form action="" method="post" class="add-comment">
      ...
      <textarea name="comment" maxlength="1000" class="comment-box" cols="30" rows="10" placeholder="yorum yap" required></textarea>
      <input type="submit" value="yorum yap" class="inline-btn" name="add_comment">
   </form>
   <?php
   }else{
   ?>
   <div class="add-comment">
      <p>Yorumunuzu eklemek veya düzenlemek için lütfen giriş yapın</p>
   ...
   <p class="comment-title">diger yorumlar</p>
   <div class="user-comments-container">
      ...
      <div class="show-comments" style="<?php if($fetch_comments['user_id'] == $user_id){echo 'order:-1;'; } ?>">
         ...
         <form action="" method="POST">
            ...
            <button type="submit" class="inline-option-btn" name="open_edit_box">yorumu düzenle</button>
